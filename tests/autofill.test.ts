...
// 한국 인스타 캡션은 취향대로 고른 장식 기호를 항목 머리로 쓴다. 목록을 미리
// 못 박으면 매번 새 기호에 당한다. 실제 게시물(서촌 낮술 코스)에서 가져온 모양.
test('캡션이 쓰는 장식 기호를 찾아내 항목을 나눈다', () => {
  const caption = [
    '(저장) 평일 휴무 서촌 낮술 코스',
    '',
    '\u{169B9}안주마을 2호점',
  ...
  const items = splitListItems(caption);
  assert.deepEqual(items.map((i) => i.name), ['안주마을 2호점', '서촌곳간', '경복궁 야간관람']);
  assert.match(items[1].memo, /감태계란말이김밥/u);
});

// 기호가 한 번만 나오면 목록이 아니라 그냥 장식이다.
test('한 번만 쓰인 기호는 항목 머리로 보지 않는다', () => {
  const caption = ['\u{169B9}오늘의 한마디', '그냥 산책했어요', '날씨가 좋았다'].join('\n');
  assert.deepEqual(splitListItems(caption), []);
});
