...
| 종류 | 캡션 키워드 — "소금빵"→베이커리, "웨이팅"→맛집, "탑노트"→향수 |
| 여러 개 | 번호 목록(`1.` `①`) → 불릿 목록(`-` `•`) → 브랜드 멘션 블록 순으로 쪼갠다 |
| 지역 | 캡션 키워드 — "애월"→제주, "파주"→경기 |
| 인스타 스크린샷 | 저장 시 게시물의 `og:image` 를 받아 대표 사진 1장으로 붙인다 (한 건일 때만) |

이름은 틀리더라도 일단 채운다. 빈 칸을 새로 쓰는 것보다 고치는 게 빠르다.
첫 줄을 쓸 때는 `(저장•공유)` 같은 말머리, 괄호 부연, 이모지, 멘션, 해시태그를
걷어내고 첫 문장까지만 남긴다.

여러 개로 쪼갤 때는 번호 목록을 먼저 본다. 번호가 1부터 연속으로 올라갈 때만
목록으로 인정하고, 없을 때만 불릿을 본다. 순서가 반대면 `-우유망고 10,900원`
같은 가격 줄이 항목 이름이 된다. 캡션에 목록이 없어도 폼에서
**여러 개로 나눠서 등록하기** 로 직접 줄을 추가할 수 있다.
캐러셀 이미지에만 이름이 있는 모음 게시물이 흔하다.

번호도 불릿도 없을 때 마지막으로 캡션 끝의 **브랜드 멘션 블록**을 본다.
...
큐레이션 계정이 소개한 브랜드를 캐러셀 순서대로 멘션해두는 관습이 있어서, 줄 개수와
브랜드가 슬라이드와 맞는 경우가 많다. 제품명은 이미지 안에 있어 줄 수 없으므로 이름은
계정 핸들 그대로 채우고 사용자가 고친다. 근거가 약한 규칙이라 오탐을 막는 조건이 세 개다.

- **줄 전체가 멘션 하나**여야 한다 — 본문 속 `(@fragranticaofficial)에서` 는 출처지 목록이 아니다
- **연속으로 이어진 덩어리**여야 한다 — 흩어진 태그는 목록이 아니다
- **세 개 이상**이어야 한다 — 사진 출처나 친구 태그 한둘과 가르는 선

캐러셀 이미지를 서버가 직접 받아올 방법은 없다. Meta 의 DM 웹훅은 첨부를 한 장(표지)만
주고 `og:image` 도 표지 한 장이라, 2번 슬라이드부터는 서버에 도착하지 않는다.
제품명까지 자동으로 채우려면 스크린샷을 OCR 하는 수밖에 없고, 그건 아직 넣지 않았다.

향수 모음처럼 이미지 한 장이 곧 제품 하나인 게시물은 캡션에 목록이 없다.
그래서 **＋ 사진으로 줄 추가** 로 캐러셀을 넘기며 찍은 스크린샷을 한꺼번에 고르면
사진 수만큼 줄이 생기고, 사진은 각 줄로 만들어진 항목에 한 장씩 따로 붙는다
(줄 왼쪽 네모를 눌러 한 줄만 바꿀 수도 있다). 게시물 표지 `og:image` 를 자동으로
붙이지 않는 것과 같은 이유로 — 표지 한 장이 다섯 개 모두의 사진인 척하면 안 된다 —
사진과 항목은 순서로 짝지어 저장한다. 이름이 빈 줄은 저장되지 않으므로 폼이 미리 알려준다.
