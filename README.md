...
3. 대시보드 → Instagram → **Webhooks** 화면에서 `messages` 필드가 **구독됨**인지 본다.
   콜백 URL 검증만 하고 필드 구독 버튼을 누르지 않은 경우가 흔하다.
   같은 화면의 **테스트 전송**으로 샘플 이벤트를 쏴 보면 콜백 경로만 따로 검증할 수 있다.

4. **계정 단위 구독**이 걸려 있는지 확인한다. 3번(앱 단위 필드 구독)과 다른 설정이다.
